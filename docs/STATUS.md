...
- **Hardening pass (from a code review)** — three `brain.py` fixes: (1) **auto-recall relevance gate** — the hook no longer injects the raw top-k; results are filtered by an absolute floor + a relative band off the top hit (`BRAIN_RECALL_MIN_SCORE`/`BRAIN_RECALL_REL`, fail-open), and trivial prompts (greetings/acks/bare continuations) skip recall entirely. Scores on this engine are small/compressed (good hits ~0.01–0.02), so the gate is relative-first and the defaults are conservative — **calibrate per-brain**. (2) **auto-distilled facts are tagged** distinctly (`—[auto-captured by …]` + `source:"auto-distill"` + `tags:["auto-distill"]`) vs human `—[saved by …]`, so machine inferences are filterable/reviewable and not mistaken for vouched facts. (3) **scrub hardened** — added GitHub (`gh[posru]_`), Slack (`xox[baprs]-`), JWT, PEM private-key blocks, and `scheme://user:PASS@host` (password-only redaction) to the regex backstop behind the LLM "no secrets" instruction.
